...
class Berman83(object):
    """
    The Berman 1983 PhD thesis quaternary model of the system CaO, MgO, Al2O3, and SiO2.
    ...
    def __init__(self,compselect=None,T=298.15,P=1,path=None,ext=''):
        ...
        Arguments:
          ...
          compselect       List of system components you want to restrict to. You
                           can choose from CaO, MgO, Al2O3, and SiO2. If no selection
        ...
        self.T          = T
        self.P          = P
        self.components = ["CaO","MgO","Al2O3","SiO2"]
    ...
    def get_mu0_at_T(self,mdb,mineral,T):
        ...
        if 'a' in mdb.columns:
            ...
            if not hasattr(self,'berman88'):
                ...
                self.berman88 = Berman88(self.compselect,T)
    ...
    def DaG(self,formula,phase,T,P=1.,name=None):
        ...
        Note about liquids:
          ...
          In the model here, we have liquid data only for the system components (MgO, SiO2, Al2O3, CaO).
          The liquid versions of all other solid/crystal phases must be calculated using the Margules
          model. But it has to be kept in mind that e.g. "liquid Mg2SiO4" is just a "liquid mixture
          of MgO and SiO2 at a ratio of 2/3 to 1/3". So it is constructed/computed, not obtained from
          a table.
